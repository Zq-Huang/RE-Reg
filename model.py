...
class ED_lv1(nn.Module):
    def __init__(self, in_channel, n_classes, start_channel, is_train=True, imgshape=(160, 192, 144), range_flow=0.4):
        ...
        self.rigid = rigid_flow((224,224,64))
    ...
    def forward(self, x, y, x_seg, y_seg):
        rigid = self.rigid.forward(x_seg, y_seg).float().cuda()

        cat_input = torch.cat((x, y, rigid), 1)
...
class ED_lv2(nn.Module):
    ...
    def forward(self, x, y, x_seg, y_seg):
        ...
        lvl1_disp, _, _, lvl1_v, lvl1_embedding = self.model_lvl1(x, y, x_seg, y_seg )
...
class ED_lv3(nn.Module):
    def __init__(self, in_channel, n_classes, start_channel, is_train=True, imgshape=(160, 192, 144), range_flow=0.4,
                 model_lvl2=None):
        super(ED_lv3, self).__init__()
        self.in_channel = in_channel
        self.n_classes = n_classes
        self.start_channel = start_channel
    ...
    def forward(self, x, y, x_seg, y_seg):
        ...
        lvl2_disp, _, _, lvl2_v, lvl1_v, lvl2_embedding = self.model_lvl2(x, y,  x_seg, y_seg)
...
class SpatialTransform_bil(nn.Module):
    def __init__(self):
        super(SpatialTransform_bil, self).__init__()
